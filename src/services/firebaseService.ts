 ...
 */
export const getDataFirestore = async (dbNames : string[], type : "nodes" | "links") => {
    try {
        ...
        if (type === "nodes") {
            ...
            results = snapshots.flatMap((querySnapshot) => querySnapshot.docs.map((doc) => {
            ...
            debugLog("debug", `Nodos obtenidos desde Firestore(${dbNames}): `, results);
        } else if (type === "links") {
            results = snapshots.flatMap((querySnapshot) => querySnapshot.docs.map((doc) => {
                const docData = doc.data();
...
export const getIndex = async () => {
    try {
        const querySnapshot = await getDocs(collection(database, tableNameDB.indexGlobal));
 ...
 * @param data.idNewNode - Identificador numérico único del nodo dentro del grafo.
...
export const addData = async (data : NodeInsertData) => {
    ...
    try {
        ...
        await updateDoc(doc(database, tableNameDB.indexGlobal, tableNameDB.indexGlobalID), {idNewNode});
        ...
        debugLog("info", "Nodo agregado:", name_es, name_en, idNewNode);
        debugLog("info", "Link agregado: Source:", nodeSource === 1 ? "Nodo sin conexión {1}" : nodeSource);
    } catch (error) {
...
export const addNodeTaxonomy = async (data : NodeTaxonomyInsert) => {
    try {
        await addDoc(collection(database, tableNameDB.nodeTaxonomy), {
